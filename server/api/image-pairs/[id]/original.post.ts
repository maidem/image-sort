...
export default defineEventHandler(async (event) => {
  ...
  const sql = useDb();
  const [pair] =
    await sql`SELECT id, original_filename FROM image_pairs WHERE id = ${id}`;
  if (!pair)
  ...
  const part = parts.find((p) => p.name === "original");
  ...
  if (pair.original_filename) {
    const oldPath = resolve(uploadPath, pair.original_filename as string);
  ...
  await sql`UPDATE image_pairs SET original_filename = ${filename} WHERE id = ${id}`;
